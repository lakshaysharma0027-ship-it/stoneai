    ...
    <main className="stone-home">
      ...
      <section className="section cta-band-section">
        <div className="container cta-band">
          <div>
            <SectionLabel tag="Get started" />
            <h2>Build your website with AI.</h2>
            <p>Generate a production-ready site, refine it visually, and publish on your domain—free to start.</p>
          </div>
          <div className="cta-band-actions">
            <Link href="/signup" className="btn btn-primary">Build Your Website With AI</Link>
            <Link href="/signup" className="btn btn-ghost">Generate A 3D Website</Link>
            <Link href="/pricing" className="btn btn-ghost">Start Free Trial</Link>
          </div>
        </div>
      </section>
      ...
      <style>{`
        ...
        @media(max-width:900px){.hero{min-height:auto;padding-top:140px}.float-stack{display:none}.hero-prompt{flex-direction:column;align-items:stretch}.trust-inner,.split-head,.template-head,.media-grid,.video-grid,.faq-grid,.cta-band{grid-template-columns:1fr}.trust-inner{grid-template-columns:1fr 1fr}.features-grid,.publish-grid,.testimonial-grid{grid-template-columns:1fr}.gen-demo-body{grid-template-columns:1fr}.gen-panel{border-right:0;border-bottom:1px solid var(--line)}.pricing-band,.cta-band{flex-direction:column;align-items:flex-start}.cta-band-actions{justify-content:flex-start}.hero h1{font-size:4rem}.typing-line{font-size:14px}.gallery-item,.gallery-item:nth-child(2),.gallery-item:nth-child(3){transform:none}}
