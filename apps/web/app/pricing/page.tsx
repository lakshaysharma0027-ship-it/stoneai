  ...
  return (
    <main className="pricing-page">
      <nav className="pricing-nav">
        <Link href="/" className="brand"><StoneLogo size={32} /></Link>
        <div><Link href="/login">Login</Link><Link href="/signup" className="start">Start free</Link></div>
      </nav>
      ...
      <style>{`
        ...
        @media(max-width:1100px){.cards-grid{grid-template-columns:repeat(2,1fr)}.enterprise-cta{flex-direction:column;align-items:flex-start}}@media(max-width:700px){.pricing-nav{height:auto;padding-top:18px}.cards-grid,.examples-grid{grid-template-columns:1fr}.pricing-hero h1{font-size:4rem}.enterprise-cta h2{font-size:32px}}
      `}</style>
    </main>
  );
}
